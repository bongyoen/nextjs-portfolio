...
export class Provider {
	...
	static getService<Service extends ObjectLiteral>(constructor: new (...args: any[]) => Service): Service {
		if (!Provider.services.has(constructor.name)) Provider.registerService(constructor);

		return Provider.services.get(constructor.name) as Service;
	}

	static getRepository<Entity extends Class>(constructor: new (...args: any[]) => Entity): Repository<Entity> {
		if (!Provider.services.has(constructor.name)) Provider.registerRepository(constructor);
...
export function Inject<Service extends ObjectLiteral>(service: new (...args: any[]) => Service): any {
